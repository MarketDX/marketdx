...
| Which stocks pay a HIGH / RELIABLE / GROWING DIVIDEND? income / yield / "กินปันผล" / dividend aristocrats | **`screen_dividends(country?, gics?, sector?)`** | Income screener over ALL payers (not just news-covered). ≥1 scope. `order_by=yield`(default)`/streak/cagr`, `min_yield`/`max_yield` (%), `min_streak_years` (25=aristocrat). 🔴 NEVER present the yield alone — each row DECOMPOSES it (`ttm_div_yoy_pct`, `no_cut_streak_yrs`, `last_cut_year`, `cagr_5y_pct`, `week52_low`, `news`): high yield + recent cut + bad news = TRAP; high yield from a price drop + intact/growing dividend + long streak = VALUE. Follow the payload `_guide`. |
